...
class CardDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final app_card.Card
    currentCardInstance = gameState.userOwnedCards.firstWhere(
    ...
    // Get costs from GameState for the current card instance
    final int goldCost = gameState.getGoldCostForLevelUp(currentCardInstance);
    final int shardCost = gameState.getShardCostForLevelUp(currentCardInstance);
    final app_card.ShardType? requiredShardType = gameState
        .getRequiredShardTypeForLevelUp(currentCardInstance);

    // Check if level up is possible
    final bool canLevelUp =
        currentCardInstance.level < currentCardInstance.maxCardLevel &&
        goldCost != -1 &&
        shardCost != -1 &&
        requiredShardType != null &&
        (gameState.playerCurrency >= goldCost &&
            ((gameState.playerShards[requiredShardType] ?? 0) >= shardCost ||
                gameState.playerSouls >=
                    (shardCost -
                        (gameState.playerShards[requiredShardType] ?? 0))));

    // Check if enhance is possible (currently only souls implemented)
    final bool canEnhance =
        currentCardInstance.level <
        currentCardInstance
            .maxCardLevel; // Basic check, refine based on enhance logic
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            if (currentCardInstance.level < currentCardInstance.maxCardLevel &&
                goldCost != -1 &&
                shardCost != -1 &&
                requiredShardType != null) ...[
              Text(
              ...
              ElevatedButton.icon(
                ...
                onPressed: canLevelUp
                    ? () {
                        bool success = gameState.upgradeCard(
                          currentCardInstance,
                        ); // No cost parameter
                        if (success) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                "${currentCardInstance.name} upgraded to Lvl ${currentCardInstance.level}!",
                              ),
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                "Failed to upgrade. Check resources or if max level reached.",
                              ),
              ...
              const SizedBox(
                height: 10,
              ), // Spacing between Level Up and Enhance
              // Enhance Button (using Souls/Fodder)
              if (canEnhance) // Only show if not max level
                ElevatedButton.icon(
                  icon: const Icon(
                    Icons.auto_awesome_outlined,
                  ), // Example icon for Enhance
                  label: const Text("Enhance (Souls/Fodder)"),
                  onPressed: () {
                    _showEnhanceDialog(context, gameState, currentCardInstance);
                  },
                  style: ElevatedButton.styleFrom(
                    minimumSize: const Size(double.infinity, 50),
            ...
            ] else if (currentCardInstance.level >=
                currentCardInstance.maxCardLevel) ...[
              // Ensure this else only triggers if max level
              Text(
                "Max Level Reached!",
                style: Theme.of(
                  context,
            ...
            if (gameState.canEvolve(
              currentCardInstance,
            )) // Assuming canEvolve exists in GameState
              ElevatedButton.icon(
                icon: const Icon(
                  Icons.trending_up_outlined,
                ), // Or a more specific evolution icon
            ...
            if (gameState.canEvolve(
              currentCardInstance,
            )) // Add spacing if Evolve button was shown
              const SizedBox(height: 16),
            // Ascension Button
            ...
            else if (currentCardInstance.rarity ==
                    app_card.CardRarity.SUPER_RARE ||
                currentCardInstance.rarity ==
                    app_card.CardRarity.ULTRA_RARE) ...[
              // Use collection else
              Padding(
  ...
  String _getAscensionDisabledReason(app_card.Card card, GameState gameState) {
    ...
    bool hasSacrificeEvo0Card = gameState.userOwnedCards.any(
      (ownedCard) =>
    ...
    final app_card.ShardType? elementalShardType = gameState
        .getElementalShardTypeFromCardType(
          card.type,
        ); // Prefixed with app_card.
    final int elementalShardCost = 30 + (card.ascensionLevel * 15);
    // The legendaryShardCost check was removed here as the shard type itself is gone.
    ...
    if (elementalShardType != null) {
      int elementalShardsOwned =
          gameState.playerShards[elementalShardType] ?? 0;
      canAffordResources =
          elementalShardsOwned >= elementalShardCost ||
    ...
    // If none of the specific failure conditions above are met, but canAscend is still false,
    // it implies another condition handled within GameState.canAscend() itself.
    return "Ascension requirements not fully met. Check Evo level, duplicates, and shards."; // Generic fallback
  }

  // --- Ascension Dialog ---
  void _showSacrificeSelectionDialog(
    BuildContext context,
    GameState gameState,
    app_card.Card cardToAscend,
  ) {
    // Find suitable sacrifice cards (Evo 0 duplicates of the same rarity)
    final suitableSacrifices = gameState.userOwnedCards
        .where(
          (c) =>
        ...
        .toList(); // Sacrifice must be Evo 0

    if (suitableSacrifices.isEmpty) {
      // Should be caught by canAscend, but good to double check
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("No suitable cards available for sacrifice."),
        ...
        return StatefulBuilder(
          builder: (BuildContext sbfDialogContext, StateSetter setDialogState) {
            return AlertDialog(
              ...
              content: SizedBox(
                // Constrain the width of the AlertDialog's content
                width:
                    MediaQuery.of(dialogContext).size.width *
                    0.8, // Or a fixed value like 300.0
                child: Column(
                  mainAxisSize:
                  ...
                  children: [
                    Text(
                      "Ascension Costs:",
                      style: Theme.of(sbfDialogContext).textTheme.titleMedium,
                    ),
                    if (elementalShardType != null)
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(
                            "- $elementalShardCost ${elementalShardType.toString().split('.').last.replaceAll('_', ' ')} (or Souls)",
                          ),
                          Text(
                            "  (Have: ${gameState.playerShards[elementalShardType] ?? 0} Shards / ${gameState.playerSouls} Souls)",
                            style: Theme.of(
                              sbfDialogContext,
                            ).textTheme.bodySmall,
                          ),
                    ...
                    const SizedBox(height: 10),
                    Text(
                      "Select 1 Evo 0 Duplicate to Sacrifice:",
                      style: Theme.of(sbfDialogContext).textTheme.titleMedium,
                    ),
                    ...
                    SizedBox(
                      height: 200,
                      width: double
                          .infinity, // Make ListView take full width of parent SizedBox
                      child: ListView.builder(
                        shrinkWrap:
                            true, // Still useful with fixed height parent
                        itemCount: suitableSacrifices.length,
                        itemBuilder: (listContext, index) {
                          // Renamed context
                          final sacrifice = suitableSacrifices[index];
                          final bool isSelected =
                              selectedSacrifice?.id == sacrifice.id;
                          return ListTile(
                            leading: Image.asset(
                              'assets/${sacrifice.imageUrl}',
                              width: 40,
                              height: 56,
                              fit: BoxFit.contain,
                            ),
                            ...
                            subtitle: Text(
                              "Lvl: ${sacrifice.level}, Evo: ${sacrifice.evolutionLevel}, Rarity: ${sacrifice.rarity.toString().split('.').last}",
                            ),
                            trailing: isSelected
                                ? const Icon(
                                    Icons.check_circle,
                            ...
                            onTap: () {
                              setDialogState(() {
                                // Use setDialogState from the outer StatefulBuilder
                                selectedSacrifice = isSelected
                                    ? null
                                    : sacrifice;
                              });
                            },
              ...
              actions: <Widget>[
                ...
                ElevatedButton(
                  ...
                  onPressed: (selectedSacrifice != null && canAffordResources)
                      ? () {
                          bool success = gameState.ascendCard(
                            cardToAscend,
                            selectedSacrifice!.id,
                          );
                          Navigator.of(
                            sbfDialogContext,
                          ...
                          if (success) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text("${cardToAscend.name} ascended!"),
                                backgroundColor: Colors.green,
                              ),
                            );
                          } else {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  "Ascension failed. Please check conditions or shard balance.",
                                ),
                                backgroundColor: Colors.red,
                              ),
                            );
  ...
  void _showEvolutionDialog(
    BuildContext context,
    GameState gameState,
    app_card.Card cardToEvolve,
  ) {
    // Find the suitable sacrifice card (Max level, same evo level duplicate)
    final suitableSacrifice = gameState.userOwnedCards.firstWhereOrNull(
      (c) =>
          ...
          c.level == c.maxCardLevel,
    ); // Sacrifice must be max level

    if (suitableSacrifice == null) {
      // Should be caught by canEvolve, but good to double check
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "No suitable max level duplicate available for evolution.",
          ),
          backgroundColor: Colors.orange,
        ),
      );
    ...
    // Calculate shard costs based on GameState logic
    final app_card.ShardType? requiredElementalShardType = gameState
        .getElementalShardTypeFromCardType(cardToEvolve.type);
    ...
    if (requiredElementalShardType == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "Cannot determine elemental shard type for ${cardToEvolve.name}.",
          ),
          backgroundColor: Colors.orange,
        ),
      );
    ...
    bool canAffordResources = false;
    int elementalShardsOwned =
        gameState.playerShards[requiredElementalShardType] ?? 0;
    if (elementalShardsOwned >= elementalShardCostForEvolution) {
      canAffordResources = true;
    } else {
      canAffordResources =
          gameState.playerSouls >=
          (elementalShardCostForEvolution - elementalShardsOwned);
    }
      ...
      builder: (BuildContext dialogContext) {
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                "Evolve to Evo ${cardToEvolve.evolutionLevel + 1}",
                style: Theme.of(context).textTheme.titleMedium,
              ),
              const SizedBox(height: 10),
              Text("Requires:", style: Theme.of(context).textTheme.titleMedium),
              Text(
                "- 1x Max Level Evo ${cardToEvolve.evolutionLevel} Duplicate of ${cardToEvolve.name}",
              ),
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(
                    "- $elementalShardCostForEvolution ${requiredElementalShardType.toString().split('.').last.replaceAll('_', ' ')} (or Souls)",
                  ),
                  Text(
                    "  (Have: ${gameState.playerShards[requiredElementalShardType] ?? 0} Shards / ${gameState.playerSouls} Souls)",
              ...
              const SizedBox(height: 10),
              Text(
                "Sacrifice Card Found:",
                style: Theme.of(context).textTheme.titleMedium,
              ),
              ListTile(
                leading: Image.asset(
                  'assets/${suitableSacrifice.imageUrl}',
                  width: 40,
                  height: 56,
                  fit: BoxFit.contain,
                ),
                ...
                subtitle: Text(
                  "Lvl: ${suitableSacrifice.level}, Evo: ${suitableSacrifice.evolutionLevel}, Rarity: ${suitableSacrifice.rarity.toString().split('.').last}",
                ),
            ...
            ElevatedButton(
              ...
              onPressed: canAffordResources
                  ? () {
                      ...
                      bool success = gameState.evolveCard(
                        cardToEvolve,
                        suitableSacrifice.id,
                      );
                      Navigator.of(dialogContext).pop(); // Close the dialog
                      if (success) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text("${cardToEvolve.name} evolved!"),
                            backgroundColor: Colors.green,
                          ),
                        );
                      } else {
  ...
  // --- Enhance Dialog ---
  void _showEnhanceDialog(
    BuildContext context,
    GameState gameState,
    app_card.Card cardToEnhance,
  ) {
              ...
              onPressed: () {
                ...
                if (soulsToUse <= 0 /* && selectedFodderIds.isEmpty */ ) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                        "Please enter souls to use or select fodder.",
                      ),
                      backgroundColor: Colors.orange,
                    ),
                  );
                ...
                bool success = gameState.enhanceCard(
                  cardToEnhance,
                  soulsToUse:
                      soulsToUse /*, fodderCardIds: selectedFodderIds */,
                );
                Navigator.of(dialogContext).pop(); // Close the dialog
                ...
                if (success) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text("${cardToEnhance.name} enhanced!"),
                      backgroundColor: Colors.green,
                    ),
                  );
                } else {
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Text(
              "Evolution: Evo ${card.evolutionLevel}",
              style: Theme.of(
                context,
              ).textTheme.titleSmall?.copyWith(color: Colors.deepPurpleAccent),
            ),
            Text(
              "Lvl: ${card.level} / ${card.maxCardLevel}",
              style: Theme.of(context).textTheme.titleMedium,
            ),
            Padding(
              padding: const EdgeInsets.symmetric(
                vertical: 12.0,
              ), // Keep padding around the container
              child: Hero(
                // Add Hero animation for the image
                tag:
                    'inventory_card_image_${card.id}', // Unique tag for Hero animation
                child: Container(
                  // Wrap Image.asset for clipping and fixed size
            ...
            if (card.talent != null) ...[
              ...
              if (card.talent!.manaCost > 0)
                Text(
                  "Mana Cost: ${card.talent!.manaCost}",
                  style: Theme.of(
                    context,
