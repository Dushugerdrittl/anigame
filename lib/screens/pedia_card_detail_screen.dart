...
class PediaCardDetailScreen extends StatelessWidget {
    ...
    return Card(
      ...
      child: Padding(
        padding: const EdgeInsets.all(
          12.0,
        ), // Reduced internal padding of the main Card
        child: Column(
          children: [
            StarDisplayWidget(
              // Add star display
              ascensionLevel:
                  card.ascensionLevel, // Template ascension is usually 0
              rarity: card.rarity,
            ...
            Padding(
              ...
              child: Hero(
                ...
                tag:
                    'pedia_card_image_${card.id}', // Unique tag for Hero animation
                child: Container(
            ...
            if (card.talent != null) ...[
              ...
              Padding(
                ...
                child: Text(
                  ...
                  textAlign: TextAlign.center,
                  // maxLines: 2, // Consider allowing more lines or using an Expandable widget if descriptions are long
            ...
            _buildSectionTitle(
              context,
              "Base Stats (as ${card.rarity.toString().split('.').last.replaceAll('_', ' ')})",
            ),
            _buildStatRow(
              context,
              "HP:",
            ...
            if (card.rarity.index < app_card.CardRarity.SUPER_RARE.index) ...[
              _buildSectionTitle(
                context,
                "Base Stats (as Super Rare)",
                titleColor: _getRarityColor(
                  app_card.CardRarity.SUPER_RARE,
                  context,
                ),
              ),
              _displayCalculatedRarityStats(
                context,
                card,
                app_card.CardRarity.SUPER_RARE,
                isEvolved: false,
            ...
            if (card.rarity == app_card.CardRarity.SUPER_RARE ||
                card.rarity == app_card.CardRarity.ULTRA_RARE) ...[
              // const SizedBox(height: 8), // Spacing handled by blocks above if they render
              _buildSectionTitle(
                context,
                "Max Evo Stats (Evo 3, as ${card.rarity.toString().split('.').last.replaceAll('_', ' ')})",
                titleColor: _getRarityColor(card.rarity, context),
              ),
              _displayCalculatedRarityStats(
                context,
                card,
  ...
  Widget _displayCalculatedRarityStats(
    BuildContext context,
    app_card.Card originalTemplateCard,
    app_card.CardRarity targetRarity, {
