...
class _InventoryScreenState extends State<InventoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool navigateToRandomBattleAfterSelection =
        arguments?['navigateToBattle'] ?? false; // For generic battle
    final String? thenSetupFloorId = arguments?['thenSetupFloorId'] as String?;
    final int? thenSetupLevelNumber =
        arguments?['thenSetupLevelNumber'] as int?;
    ...
    List<app_card.Card> filteredCards = gameState.userOwnedCards.where((card) {
      ...
      final rarityMatches =
          _selectedRarityFilter == null || card.rarity == _selectedRarityFilter;
      final typeMatches =
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          padding: const EdgeInsets.all(
            5.0,
          ), // Reduced padding to fit (default is 8.0)
        ...
        toolbarHeight:
            30, // Adjust this value to make the AppBar smaller (default is ~56)
        ...
        children: [
          Padding(
            ...
            child: TextField(
              ...
              style: const TextStyle(fontSize: 14), // Consistent with CardPedia
              decoration: InputDecoration(
                hintText: 'Search cards by name...',
                hintStyle: const TextStyle(
                  fontSize: 14,
                ), // Consistent with CardPedia
                ...
                fillColor: Theme.of(
                  context,
                ).colorScheme.surfaceContainerHighest, // Consistent
              ...
              children: [
                Expanded(
                  child: DropdownButtonFormField<app_card.CardRarity?>(
                    decoration: InputDecoration(
                      // labelText: 'Rarity', // Using hint instead
                      hintText: 'Rarity',
                      hintStyle: const TextStyle(
                        fontSize: 13,
                ...
                Expanded(
                  child: DropdownButtonFormField<CardType?>(
                    ...
                    decoration: InputDecoration(
                      // labelText: 'Type', // Using hint instead
                      hintText: 'Type',
                      hintStyle: const TextStyle(
                        fontSize: 13,
                        color: Colors.black54,
                ...
                : GridView.builder(
                    ...
                    gridDelegate:
                        const SliverGridDelegateWithFixedCrossAxisCount(
                          ...
                          childAspectRatio:
                              0.6, // Adjusted for a more portrait card
                          crossAxisSpacing: 8,
                          mainAxisSpacing: 8,
                        ),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      bool isSelected =
                          gameState.currentlySelectedPlayerCard?.id ==
                              card.id &&
                          gameState.currentlySelectedPlayerCard == card;
                      return InkWell(
                        onTap: () async {
                          // Make async if performing multiple navigations
                          context.read<GameState>().selectCardForBattle(card);
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text("${card.name} selected!")),
                          );

                          if (thenSetupFloorId != null &&
                              thenSetupLevelNumber != null) {
                            context.read<GameState>().setupBattleForFloorLevel(
                              thenSetupFloorId,
                              thenSetupLevelNumber,
                            );
                            Navigator.pop(context); // Pop inventory
                            Navigator.pushNamed(
                              context,
                              '/battle',
                            ); // Then push battle screen
                          } else if (navigateToRandomBattleAfterSelection) {
                            context
                                .read<GameState>()
                                .setupNewRandomBattle(); // Setup a random battle
                            Navigator.pop(
                              context,
                              true,
                            ); // Pop, indicating selection for battle is done
                          } else {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
          ...
          if (navigateToRandomBattleAfterSelection &&
              thenSetupFloorId == null &&
              gameState.userOwnedCards.isNotEmpty &&
              gameState.currentlySelectedPlayerCard != null)
            Padding(
              padding: const EdgeInsets.all(16.0),
  ...
  Color _getRarityColor(app_card.CardRarity rarity, BuildContext context) {
    switch (rarity) {
      case app_card.CardRarity.COMMON:
        return app_card.kCommonColor;
      case app_card.CardRarity.UNCOMMON:
        return app_card.kUncommonColor;
      case app_card.CardRarity.RARE:
        return app_card.kRareColor;
      case app_card.CardRarity.SUPER_RARE:
        return app_card.kSuperRareColor;
      case app_card.CardRarity.ULTRA_RARE:
        return app_card.kUltraRareColor;
    }
  }
